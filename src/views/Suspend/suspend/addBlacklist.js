...
import { partnerOptionsDrop, noOfDaysOptionsDrop, saveBlacklistData } from 'services/blacklist';
...
const AddBlacklist = ({ data = {user_type: '', user_id: '', no_of_suspend_days: '', remarks: '' } }) => {
  ...
  useEffect(() => {
    const fetchNoOfDaysOptions = async () => {
      try {
        ...
        const options = response.getData;
        const transformedOptions = options.map((option) => ({
          value: option.no_of_blacklisted_days,
          label: option.no_of_blacklisted_days,
        }));
  ...
  const handleSubmit = async (values,{resetForm, setSubmitting }) => {
    try {
       setSubmitting(true);
        ...
        // console.log(values);
      const response = await saveBlacklistData(values);
  ...
  return (
    <>
      <div className="mb-8">
        <h3 className="mb-2">Blacklist</h3>
      </div>
      <Formik
                  ...
                  <div className="input-container">
                    ...
                    >
                      <div className="input-container1">
                        <FormItem
                          label="Partner"
                          invalid={errors.partCompDrop && touched.partCompDrop}
                          errorMessage={errors.partCompDrop}
                        >
                          <Field name="user_id">
                            {({ field, form }) => (
                              <Select
                      ...
                      <div className="input-container">
                        <FormItem
                          label="No. of Days"
                          invalid={errors.noOfDays && touched.noOfDays}
                          errorMessage={errors.noOfDays}
                        >
                          <Field name="no_of_blacklisted_days">
                            {({ field, form }) => (
                              <Select
