...
const FilterForm = forwardRef(({ onSubmitComplete}, ref) => {
  ...
  const handleSubmit = (values) => {
    ...
    const payload = {
      ...
      tokenKey,
      searchtype: selected,
      installer_name:values.InstallerName,
    ...
    console.log(payload)    
    dispatch(getSurveyor(payload));
  };  
  const [selected, setSelected] = useState();
  // const [DateFilterVisible, setDateFilterVisible] = useState(false);
  const [InstallerIdFilterVisible, setInstallerIdFilterVisible] = useState(false);
  const[packagenameFilterVisible ,setpackagenameFilterVisible]=useState(false);
...
//   };
  ...
  const SearchByInstallerName = () => {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-3 mt-8">
        <div className="lg:col-span-1 mt-2 ">
          <h6>Installer Name.</h6>
        </div>
        <div className="lg:col-span-2 ">
          <Field type="text" autoComplete="off" name="InstallerName" placeholder="Installer Name" component={Input} />
        </div>
      </div>
    );
  };
  const SearchByPackageName = () => {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-3 mt-8">
        <div className="lg:col-span-1 mt-2 ">
          <h6>Package Name</h6>
        ...
        <div className="lg:col-span-2 ">
          <Field type="text" autoComplete="off" name="packageName" placeholder="Package Name" component={Input} />
        </div>
      </div>
    );
  };
...
// };
  ...
  useState(() => {
    setSelected();
    setpackagenameFilterVisible(false);
    setInstallerIdFilterVisible(false);
    // setRegionFilterVisible(false);
  }, []);

  const onStatusFilterChange = (e) => {
    setSelected(e.value);
    // setDateFilterVisible(e.value === 'created_dt');
    setInstallerIdFilterVisible(e.value === 'installer_name');
    setpackagenameFilterVisible(e.value === 'package_name');
  };
        ...
        <Form>
          <FormContainer>
            <FormItem invalid={errors.name && touched.name} errorMessage={errors.name}>
              ...
              {InstallerIdFilterVisible && <SearchByInstallerName />}
              {packagenameFilterVisible && <SearchByPackageName/>} 
            </FormItem>
          </FormContainer>
        </Form>
      )}
