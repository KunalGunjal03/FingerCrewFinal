...
import { getInstaller, setTableData } from '../store/dataSlice'
...
const InstallerTable = () => {
    ...
    const fetchData = () => {
        try{
        dispatch(getInstaller({ pageIndex, pageSize, sort, query, filterData,token,tokenKey}))
        }catch(error)
        {
            console.error(error)
            return error;
        ...
        () => [
            {
                ...
                cell: (props) => {
                    ...
                    return <InstallerCloumn row={row} />
                },
            },
            {
